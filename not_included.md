...
истинные значения констант, если кто-то знает, почему используются именно эти значения, просьба написать в комментариях. Также выяснилось, что значения для каждой клавиши могут меняться со временем в диапазоне от 0x0000 до 0xF000 с шагом 0x0100. Кроме того, было установлено, что тип значений кнопок A и START — int(возможно short, но функция `SegaGenesisgetJoypadValuePort1` возвращает int), а для остальных кнопок — byte. Это говорит о том, что значения кнопок могут изменяться и не являются константами. Учитывая данные особенности, для реализации управления можно использовать два метода: 
